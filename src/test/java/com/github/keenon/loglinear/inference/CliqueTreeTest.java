...
public class CliqueTreeTest {
  ...
  @Test
  public void testCompileNormalizedModelCorrectMarginals() throws Exception {
    ConcatVector weights = new ConcatVector(1);
    weights.setDenseComponent(0, new double[]{2.0, 1.0});
    GraphicalModel model = new GraphicalModel();
    model.addFactor(new int[]{0}, new int[]{2}, assign -> {
      ConcatVector vector = new ConcatVector(2);
      if (assign[0] == 0) {
        vector.setDenseComponent(0, new double[]{1.0, 0.0});
      } else {
        vector.setDenseComponent(0, new double[]{0.0, 1.0});
      }
      return vector;
    });

    CliqueTree tree = new CliqueTree(model, weights);
    CliqueTree.MarginalResult unnormalizedSolve = tree.calculateMarginals();
    GraphicalModel normalized = tree.compileNormalizedModel();
    CliqueTree.MarginalResult normalizedSolve = new CliqueTree(normalized, new ConcatVector(0)).calculateMarginals();

    // The tests
    assertEquals("expected: " + Arrays.toString(unnormalizedSolve.marginals[0]) + " but was: " + Arrays.toString(normalizedSolve.marginals[0]),
        unnormalizedSolve, normalizedSolve);
    assertEquals(1.0, normalizedSolve.partitionFunction, 1e-5);
    assertEquals(0.7310585784, normalizedSolve.marginals[0][0], 1e-5);
    assertEquals(0.2689414213, normalizedSolve.marginals[0][1], 1e-5);
  }


  @Test
  public void testCompileNormalizedModelCorrectMarginalsWithNegativeWeights() throws Exception {
    ConcatVector weights = new ConcatVector(1);
    weights.setDenseComponent(0, new double[]{-1.0, 1.0});
    GraphicalModel model = new GraphicalModel();
    model.addFactor(new int[]{0}, new int[]{2}, assign -> {
      ConcatVector vector = new ConcatVector(2);
      if (assign[0] == 0) {
    ...
    assertEquals("expected: " + Arrays.toString(unnormalizedSolve.marginals[0]) + " but was: " + Arrays.toString(normalizedSolve.marginals[0]),
        unnormalizedSolve, normalizedSolve);
    assertEquals(1.0, normalizedSolve.partitionFunction, 1e-5);
    assertEquals(0.11920292202, normalizedSolve.marginals[0][0], 1e-5);
    assertEquals(0.88079707797, normalizedSolve.marginals[0][1], 1e-5);
  }
  ...
  @Theory
  public void testCompileModelSameFactors(@ForAll(sampleSize = 100) @From(GraphicalModelGenerator.class) GraphicalModel model,
                                          @ForAll(sampleSize = 10) @From(WeightsGenerator.class) ConcatVector weights) throws Exception {

    GraphicalModel compiled = new CliqueTree(model, weights).compileModel();
    for (GraphicalModel.Factor newFactor : compiled.factors) {
      boolean anySame = false;
      for (GraphicalModel.Factor oldFactor : model.factors) {
        boolean thisSame = true;

        // Check the shape
        if (oldFactor.getDimensions().length != newFactor.getDimensions().length) {
          continue;
        }
        for (int i = 0; i < oldFactor.getDimensions().length; ++i) {
          if (oldFactor.getDimensions()[i] != newFactor.getDimensions()[i]) {
            thisSame = false;
          }
        }
        for (int i = 0; i < oldFactor.neigborIndices.length; ++i) {
          if (oldFactor.neigborIndices[i] != newFactor.neigborIndices[i]) {
            thisSame = false;
          }
        }

        // Check the assignments
        Iterator<int[]> iter = oldFactor.fastPassByReferenceIterator();
        while (iter.hasNext() && thisSame) {
          int[] assignment = iter.next();
          double oldValue = oldFactor.getAssignmentValue(assignment, weights);
          double newValue = newFactor.getAssignmentValue(assignment, weights);
  ...
  @Theory
  public void testCompileModelSameSolve(@ForAll(sampleSize = 100) @From(GraphicalModelGenerator.class) GraphicalModel model,
                                        @ForAll(sampleSize = 10) @From(WeightsGenerator.class) ConcatVector weights) throws Exception {
    CliqueTree.MarginalResult uncompiledSolve = new CliqueTree(model, weights).calculateMarginals();
    CliqueTree.MarginalResult reSolve = new CliqueTree(model.cloneModel(), new ConcatVector(weights)).calculateMarginals();
    assertEquals(uncompiledSolve, reSolve);  // make sure solve is repeatable
    GraphicalModel compiled = new CliqueTree(model, weights).compileModel();
    CliqueTree.MarginalResult compiledSolve = new CliqueTree(compiled, new ConcatVector(0)).calculateMarginals();
    assertEquals(uncompiledSolve, compiledSolve);
  }


  private double assignmentScore(CliqueTree reference, int[] a) {
    double logScore = 0.0;
    for (GraphicalModel.Factor factor : reference.model.factors) {
      int[] assignment = new int[factor.neigborIndices.length];
      for (int i = 0; i < assignment.length; ++i) {
        assignment[i] = a[factor.neigborIndices[i]];  // The assignment from A
      }
      logScore += factor.getAssignmentValue(assignment, reference.weights);
    }
    return logScore;
  }

  private void mapOptimal(CliqueTree reference, int[] a) {
    int[] variableSizes = reference.model.getVariableSizes();
    double baseline = assignmentScore(reference, a);
    // Singleton mutations
    for (int i = 0; i < a.length; ++i) {
      for (int val = 0; val < variableSizes[i]; ++val) {
        if (val != a[i]) {
          int saved = a[i];
          a[i] = val;
          double candidate = assignmentScore(reference, a);
          a[i] = saved;
          assertTrue("Found a better MAP assignment of " + i + "=" + val, candidate < baseline + 1e-5);
        }
      }
    }
    // Pairwise mutations
    for (int i = 0; i < a.length; ++i) {
      for (int k = 0; k < a.length; ++k) {
        if (i == k) { continue; }
        for (int valI = 0; valI < variableSizes[i]; ++valI) {
          for (int valK = 0; valK < variableSizes[k]; ++valK) {
            if (valI != a[i] && valK != a[k]) {
              int savedI = a[i];
              int savedK = a[k];
              a[i] = valI;
              a[k] = valK;
              ...
              a[i] = savedI;
              a[k] = savedK;
              assertTrue("Found a better MAP assignment of " + i + "=" + valI + "; " + k + " = " + valK, candidate < baseline + 1e-5);
            }
          }
        }
      }
  ...
  /**
   * Checks to make sure the two assignments below are either (1) the same, or (2) have the same
   * score (to be robust to tiebreaking)
   *
   * @param reference the clique tree encoding the model and weights.
   * @param a The first MAP assignment
   * @param b The second MAP assignment
   */
  private void sameMAP(CliqueTree reference, int[] a, int[] b) {
    if (!Arrays.equals(a, b)) {
      double logScoreA = 0.0;
      double logScoreB = 0.0;
      for (GraphicalModel.Factor factor : reference.model.factors) {
        int[] assignment = new int[factor.neigborIndices.length];
        for (int i = 0; i < assignment.length; ++i) {
          assignment[i] = a[factor.neigborIndices[i]];  // The assignment from A
        }
        logScoreA += factor.getAssignmentValue(assignment, reference.weights);
        for (int i = 0; i < assignment.length; ++i) {
          assignment[i] = b[factor.neigborIndices[i]];  // The assignment from B
        }
        logScoreB += factor.getAssignmentValue(assignment, reference.weights);
      }
      assertTrue("MAP assignment disagrees; MAP log score = " + logScoreA + " vs " + logScoreB + "\n" +
          "a = " + Arrays.toString(a) + "\n" +
          "b = " + Arrays.toString(b),
          Math.abs(logScoreA - logScoreB) < 1e-5);
    }
  }
  ...
  /*
  @Theory
  public void testCopiedModelSameMAP(@ForAll(sampleSize = 1000) @From(GraphicalModelGenerator.class) GraphicalModel model,
                                     @ForAll(sampleSize = 10) @From(WeightsGenerator.class) ConcatVector weights) throws Exception {
    CliqueTree original = new CliqueTree(model, weights);
    int[] solve = original.calculateMAP();
    int[] simpleResolve = original.calculateMAP();
    sameMAP(original, solve, simpleResolve);
    CliqueTree copy = new CliqueTree(model.cloneModel(), weights.deepClone());
    int[] resolve = copy.calculateMAP();
    sameMAP(original, solve, resolve);
    mapOptimal(original, solve);
  }


  @Theory
  public void testCompileModelSameMAP(@ForAll(sampleSize = 1000) @From(GraphicalModelGenerator.class) GraphicalModel model,
                                        @ForAll(sampleSize = 10) @From(WeightsGenerator.class) ConcatVector weights) throws Exception {
    int[] uncompiledSolve = new CliqueTree(model, weights).calculateMAP();
    GraphicalModel compiled = new CliqueTree(model, weights).compileModel();
    int[] compiledSolve = new CliqueTree(compiled, new ConcatVector(0)).calculateMAP();
    assertArrayEquals(uncompiledSolve, compiledSolve);
    mapOptimal(new CliqueTree(model, weights), compiledSolve);
  }
  */
  ...
  private void randomlyMutateGraphicalModel(GraphicalModel model, Random r) {
    if (r.nextBoolean() && model.factors.size() > 1) {
      ...
      for (GraphicalModel.Factor f : model.factors) {
        for (int j = 0; j < f.neigborIndices.length; j++) {
          ...
          if (r.nextDouble() > 0.3 || attachVar == -1) {
            attachVar = k;
            attachVarSize = f.getDimensions()[j];
          }
        }
      }
      ...
      // Randomly decide between using a ConcatVector factor, or a static factor
      if (r.nextBoolean()) {
        GraphicalModel.VectorFactor binary = model.addFactor(new int[]{newVar, attachVar}, new int[]{newVarSize, attachVarSize}, (assignment) -> {
        ...
        // This cooks the array for us the first time we create it
        model.addStaticFactor(new int[]{newVar, attachVar}, new int[]{newVarSize, attachVarSize}, (assignment) -> r.nextDouble());
      }
    }
  }
  ...
  public void checkMAPAgainstBruteForce(GraphicalModel model, ConcatVector weights, CliqueTree inference) {
    ...
    int[] mapAssn = new int[bruteForce.neighborIndices.length];
    int[] bruteForceAssn = new int[bruteForce.neighborIndices.length];
    for (int i = 0; i < bruteForce.neighborIndices.length; i++) {
      mapAssn[i] = map[bruteForce.neighborIndices[i]];
      bruteForceAssn[i] = mapValueAssignment[bruteForce.neighborIndices[i]];
    }
    double mapValue = bruteForce.getAssignmentValue(mapAssn);
    double bruteForceValue = bruteForce.getAssignmentValue(bruteForceAssn);

    if (!Arrays.equals(mapValueAssignment, map) && mapValue != bruteForceValue) {
      System.err.println("---");
      ...
      System.err.println("MAP               : " + Arrays.toString(map) + " = " + mapValue);
      System.err.println("Brute force map   : " + Arrays.toString(mapValueAssignment) + " = " + bruteForceValue);
      System.err.println("Forced assignments: " + Arrays.toString(forcedAssignments));

      // For breakpoint convenience
      inference.calculateMAP();
    }

    // Check that we found the same best point, cause sometimes we tiebreak in different ways
    assertEquals(bruteForceValue, mapValue, 1.0e-9);
  }
    ...
    public GraphicalModel generate(SourceOfRandomness sourceOfRandomness, GenerationStatus generationStatus) {
      ...
      // common in practice, so worth testing densely
        ...
        for (int i = 0; i < variableSizes.length; i++) {
          ...
          GraphicalModel.VectorFactor unary = model.addFactor(new int[]{i}, new int[]{variableSizes[i]}, (assignment) -> {
            ConcatVector features = new ConcatVector(CONCAT_VEC_COMPONENTS);
            for (int j = 0; j < CONCAT_VEC_COMPONENTS; j++) {
              if (sourceOfRandomness.nextBoolean()) {
                features.setSparseComponent(j, sourceOfRandomness.nextInt(CONCAT_VEC_COMPONENT_LENGTH), 2.0 * (sourceOfRandomness.nextDouble() - 0.5));
              } else {
          ...
          if (i < variableSizes.length - 1) {
            GraphicalModel.VectorFactor binary = model.addFactor(new int[]{i, i + 1}, new int[]{variableSizes[i], variableSizes[i + 1]}, (assignment) -> {
              ConcatVector features = new ConcatVector(CONCAT_VEC_COMPONENTS);
              for (int j = 0; j < CONCAT_VEC_COMPONENTS; j++) {
                if (sourceOfRandomness.nextBoolean()) {
                  features.setSparseComponent(j, sourceOfRandomness.nextInt(CONCAT_VEC_COMPONENT_LENGTH), 2.0 * (sourceOfRandomness.nextDouble() - 0.5));
      ...
      for (GraphicalModel.Factor f : model.factors) {
        for (int i = 0; i < f.neigborIndices.length; i++) {
          if (sourceOfRandomness.nextDouble() > 0.8) {
            int obs = sourceOfRandomness.nextInt(f.getDimensions()[i]);
      ...
      for (int[] assignment : table) {
        ...
        for (int x = 0; x < CONCAT_VEC_COMPONENTS; x++) {
          if (randomness.nextBoolean()) {
            ...
            for (int y = 0; y < val.length; y++) {
              val[y] = 2.0 * (randomness.nextDouble() - 0.5);
            }
            v.setDenseComponent(x, val);
          }
        }
