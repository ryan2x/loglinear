package io.github.keenon.loglinear.inference;
...
import io.github.keenon.loglinear.model.ConcatVector;
import io.github.keenon.loglinear.model.ConcatVectorTable;
import io.github.keenon.loglinear.model.GraphicalModel;
...
public class CliqueTreeTest {
    ...
    public void testCalculateMarginals(@ForAll(sampleSize = 50) @From(GraphicalModelGenerator.class) GraphicalModel model,
        ...
        if (bruteForce != null) {
            ...
            marginals: for (GraphicalModel.Factor f : model.factors) {
                assertTrue(result.jointMarginals.containsKey(f));

                TableFactor bruteForceJointMarginal = bruteForce;
                outer: for (int n : bruteForce.neighborIndices) {
                    for (int i : f.neigborIndices) if (i == n) {
                        continue outer;
                    }
                ...
                outer: for (int[] assignment : result.jointMarginals.get(f)) {
    ...
    @Theory
    public void testCalculateMap(@ForAll(sampleSize = 50) @From(GraphicalModelGenerator.class) GraphicalModel model,
                                 @ForAll(sampleSize = 2) @From(WeightsGenerator.class) ConcatVector weights) throws Exception {
